...
void ir_begin() {
    ...
    if (qtr.calibrationOn.minimum == nullptr || qtr.calibrationOn.maximum == nullptr) {
        serial_logf("[m7-ir] calibration allocation failed\n");
        return;
    }
...
void ir_update(uint32_t now_ms) {
    ...
    if (duration_ms > 10) {
        serial_logf("[m7-ir] read took %lums pos=%u\n",
