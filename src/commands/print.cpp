...
static void print_stat() {
    ...
    wifi_tx("Heap:    %10lu  %10lu  %10lu  %10lu\n",
    heap.reserved_size,
    heap.current_size,
    heap.max_size,
...
static void print(int argc, char** argv) {
    ...
    if (argc != 2) {
        wifi_tx("print needs exactly 1 arguments.\n");
        return;
    }
    ...
    if (strcmp(argv[1], "ir") == 0) {
        ...
        // show dist
        wifi_tx("Dist: Front: %dcm, Left %dcm, Right: %dcm\n",
        ...
        // show orien
        wifi_tx("orientation: roll: %.2fdeg pitch: %.2fdeg, yaw: %.2f\n",
