...
int USBHost::findDevice(USBDeviceConnected * dev)
{
    for (int i = 0; i < MAX_DEVICE_CONNECTED; i++) {
        if (dev == &devices[i]) {
            printf("[USBHost] findDevice: dev=%p matched devices[%d]=%p\r\n", dev, i, &devices[i]);
            return i;
        }
    }
    printf("[USBHost] findDevice: dev=%p NOT FOUND in devices[0..%d]\r\n", dev, MAX_DEVICE_CONNECTED - 1);
    return -1;
}
...
USB_TYPE USBHost::getConfigurationDescriptor(USBDeviceConnected * dev, uint8_t * buf, uint16_t max_len_buf, uint16_t * len_conf_descr)
{
    ...
    // Some 2.4G wireless receivers fail on control reads > 64 bytes; retry with 64
    if (res != USB_TYPE_OK && total_conf_descr_length > 64) {
        res = controlRead(  dev,
                            USB_DEVICE_TO_HOST | USB_RECIPIENT_DEVICE,
                            GET_DESCRIPTOR,
                            (CONFIGURATION_DESCRIPTOR << 8) | (0),
                            0, buf, 64);
        if (res == USB_TYPE_OK && len_conf_descr) {
            *len_conf_descr = 64;
        }
    }
    return res;
...
USB_TYPE USBHost::enumerate(USBDeviceConnected * dev, IUSBEnumerator* pEnumerator)
{
    ...
    do {
        ...
        printf("[USBHost] enumerate: findDevice returned %d\r\n", index);

        // third step: get the whole device descriptor to see vid, pid
        res = getDeviceDescriptor(dev, data, DEVICE_DESCRIPTOR_LENGTH);

        if (res != USB_TYPE_OK) {
            printf("[USBHost] enumerate: getDeviceDescriptor failed, res=%d\r\n", (int)res);
            return res;
        }
        printf("[USBHost] enumerate: getDeviceDescriptor OK\r\n");
        ...
        if (res != USB_TYPE_OK) {
            printf("[USBHost] enumerate: getConfigurationDescriptor failed, res=%d\r\n", (int)res);
            return res;
        }
        printf("[USBHost] enumerate: getConfigurationDescriptor OK, len=%d\r\n", total_conf_descr_length);
        ...
        if (!dev->isEnumerated()) {
            printf("[USBHost] enumerate: setting configuration 1\r\n");
            // sixth step: set configuration (only 1 supported)
            res = setConfiguration(dev, 1);

            if (res != USB_TYPE_OK) {
                printf("[USBHost] enumerate: setConfiguration failed, res=%d\r\n", (int)res);
                return res;
            }
            printf("[USBHost] enumerate: setConfiguration OK\r\n");
        }

        dev->setEnumerated();
        printf("[USBHost] enumerate: success, returning OK\r\n");
