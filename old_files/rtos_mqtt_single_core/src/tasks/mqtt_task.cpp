...
const char* wifi_security_name(nsapi_security_t security) {
    switch (security) {
    case NSAPI_SECURITY_NONE: return "open";
    ...
    case NSAPI_SECURITY_UNKNOWN: return "unknown";
    default: return "other";
    }
}

void scan_wifi_networks(WiFiInterface* wifi, bool force = false) {
    static uint32_t last_scan_ms = 0;
    uint32_t now = millis();
    if (!force && last_scan_ms != 0 && now - last_scan_ms < WIFI_SCAN_INTERVAL_MS) {
        return;
    }
    last_scan_ms = now;

    if (wifi == nullptr) {
        mqtt_logf("WiFi scan skipped: interface is null\n");
        return;
    }

    WifiBusyGuard guard;
    WiFiAccessPoint aps[WIFI_SCAN_MAX_APS];
    mqtt_logf("Scanning WiFi networks...\n");
    int count = wifi->scan(aps, WIFI_SCAN_MAX_APS);
    if (count < 0) {
        mqtt_logf("WiFi scan failed rc=%d\n", count);
        return;
    }

    mqtt_logf("Found %d WiFi networks%s\n", count, count >= WIFI_SCAN_MAX_APS ? " (truncated)" : "");
    for (int i = 0; i < count; i++) {
        mqtt_logf(" %2d: ssid=%s rssi=%d security=%s channel=%u\n",
        i + 1,
        aps[i].get_ssid(),
        aps[i].get_rssi(),
        wifi_security_name(aps[i].get_security()),
        aps[i].get_channel());
    }
}
...
void make_status_topic(char* out, size_t len) {
    snprintf(out,
    len,
...
int publish_payload(MqttClient& client, const char* topic, const char* payload, bool retained = false, MQTT::QoS qos = MQTT::QOS0) {
...
void publish_status(MqttClient& client, const char* status) {
    char topic[96];
    make_status_topic(topic, sizeof(topic));

    const int rc = publish_payload(client, topic, status, true, MQTT::QOS1);
    if (rc != MQTT::SUCCESS) {
        mqtt_logf("status publish failed status=%s rc=%d\n", status, rc);
    }
}
...
void drain_tx_queue(MqttClient& client) {
    ...
    if (mail_wifi_tx.empty() || !client.isConnected()) {
        return;
    }
...
bool connect_mqtt(MqttClient& client, MbedMqttNetwork& network) {
    ...
    make_status_topic(status_topic, sizeof(status_topic));
    ...
    connect_data.will.topicName.cstring = status_topic;
    connect_data.will.message.cstring   = const_cast<char*>("offline");
    connect_data.will.retained          = 1;
    connect_data.will.qos               = MQTT::QOS1;
...
void func_mqtt() {
    ...
    while (true) {
        ...
        while (client.isConnected()) {
            const int rc = client.yield(MQTT_YIELD_TIMEOUT_MS);
            if (rc != MQTT::SUCCESS) {
