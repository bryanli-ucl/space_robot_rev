...
int16_t normalize_distance_cm(float value) {
    if (value <= 0.0f || value > CONFIG::ULTRASONIC_MAX_DISTANCE_CM) {
        return -1;
    }
...
void func_sensors() {
    ...
    if (CONFIG::ENABLE_SENSOR_MOUSE) {
        ...
        serial_tx("Mouse ready\n");
    }

    serial_tx("Sensors task ready: ir=%d ultrasonic=%d sunlight=%d mouse=%d\n",
              CONFIG::ENABLE_SENSOR_IR_ARRAY,
              CONFIG::ENABLE_SENSOR_ULTRASONIC,
              CONFIG::ENABLE_SENSOR_SUN_LIGHT,
              CONFIG::ENABLE_SENSOR_MOUSE);

    constexpr float us_temperature = 25.f;

    while (1) {
        if (wifi_busy) {
            static uint32_t last_wifi_pause_log_ms = 0;
            if (millis() - last_wifi_pause_log_ms >= 2000 || last_wifi_pause_log_ms == 0) {
                last_wifi_pause_log_ms = millis();
                serial_tx("Sensors paused while WiFi is busy\n");
            }
        ...
        static uint32_t last_ultrasonic_duration_ms = 0;
        if (CONFIG::ENABLE_SENSOR_ULTRASONIC &&
            (millis() - last_ultrasonic_ms >= CONFIG::ULTRASONIC_INTERVAL_MS || last_ultrasonic_ms == 0)) {
            last_ultrasonic_ms = millis();
            uint32_t measure_start_ms = millis();

            if (ultrasonic_index == 0) {
                dist_front = normalize_distance_cm(usf.measureDistanceCm(us_temperature));
            } else if (ultrasonic_index == 1) {
                dist_left = normalize_distance_cm(usl.measureDistanceCm(us_temperature));
            } else {
                dist_right = normalize_distance_cm(usr.measureDistanceCm(us_temperature));
            }

            last_ultrasonic_duration_ms = millis() - measure_start_ms;
        ...
        static uint32_t last_sensor_status_ms = 0;
        if (millis() - last_sensor_status_ms >= 5000 || last_sensor_status_ms == 0) {
            last_sensor_status_ms = millis();
            serial_tx("Sensors status: wifi_busy=%d ir_pos=%u dist=%d/%d/%d us_dt=%lums sunlight=%u mouse=%ld/%ld/%d\n",
                      wifi_busy,
                      ir_pos,
                      dist_front,
                      dist_left,
                      dist_right,
                      static_cast<unsigned long>(last_ultrasonic_duration_ms),
                      sun_light,
                      static_cast<long>(mx),
                      static_cast<long>(my),
                      mz);
...
void func_imu() {
    ...
    serial_tx("IMU task ready: ok=%d yaw_ready=%d\n", imu_ok, imu_yaw_ready);
    ...
    while (1) {
        ...
        static uint32_t last_imu_status_ms = 0;
        if (millis() - last_imu_status_ms >= 5000 || last_imu_status_ms == 0) {
            last_imu_status_ms = millis();
            serial_tx("IMU status: ok=%d yaw_ready=%d yaw=%.2f\n", imu_ok, imu_yaw_ready, imu_yaw_deg);
        }
...
void func_rfid() {
    ...
    while (1) {
        ...
        static uint32_t last_rfid_status_ms = 0;
        if (millis() - last_rfid_status_ms >= 5000 || last_rfid_status_ms == 0) {
            last_rfid_status_ms = millis();
            serial_tx("RFID status: detected_uid=%lu\n", static_cast<unsigned long>(detected_uid));
        }
