...
class MiniBroker:
    ...
    async def handle_client(
        ...
        try:
            while True:
                packet_type, flags, payload = await self.read_packet(reader)
                print(
    ...
    async def read_packet(self, reader: asyncio.StreamReader) -> tuple[int, int, bytes]:
        fixed = (await asyncio.wait_for(reader.readexactly(1), timeout=10))[0]
        remaining_length = 0
        multiplier = 1

        while True:
            byte = (await asyncio.wait_for(reader.readexactly(1), timeout=10))[0]
    ...
    async def handle_connect(self, client: Client, payload: bytes) -> None:
        ...
        print(
            f"[connect-packet] protocol={protocol} level={level} flags=0x{flags:02x} client_id={client.client_id}",
            flush=True,
        )
