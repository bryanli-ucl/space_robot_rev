...
Type a robot bash command into this terminal and press Enter to publish it:
    > start
    > state
    > move 40 0 0 1000
"""
...
class MiniBroker:
    ...
    async def stdin_command_loop(
        ...
        topic = f"lab/g/{group_id}/from/{from_id}/to/{target}"
        print(
            f"Command input enabled: publish to {topic}. "
            "Type bash commands, or blank line to skip.",
            flush=True,
        )
        ...
        while True:
            ...
            payload = command if raw else f"type=command cmd={command}"
            await self.publish(topic, payload.encode("utf-8"), retain=False)
...
async def main() -> None:
    ...
    parser.add_argument(
        "--command-target",
        default="12",
        help="Robot board ID for stdin commands, or 'all' for group broadcast",
    )
    parser.add_argument(
        "--command-from",
        default="server",
        help="Sender board ID used in command topics",
    )
    parser.add_argument(
        "--no-command-input",
        action="store_true",
        help="Disable interactive stdin command publishing",
    )
    parser.add_argument(
        "--raw-command",
        action="store_true",
        help="Publish raw command text instead of 'type=command cmd=...'",
    )
    ...
    if not args.no_command_input:
        asyncio.create_task(
            broker.stdin_command_loop(
                group_id=args.group,
                from_id=args.command_from,
                target=args.command_target,
                raw=args.raw_command,
            )
        )
